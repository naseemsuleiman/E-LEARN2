...
import { checkEnrollment, unenrollFromCourse } from '../services/lms';
...
function Courses() {
  ...
  const handleEnroll = async (courseId) => {
    ...
    setEnrolled({ ...enrolled, [courseId]: true });
    window.showToast && window.showToast('Enrolled successfully!', 'success');
  };

  const handleUnenroll = async (courseId) => {
    await unenrollFromCourse(courseId);
    setEnrolled({ ...enrolled, [courseId]: false });
    window.showToast && window.showToast('Unenrolled from course.', 'success');
  };
                ...
                {userRole === 'student' && (
                  enrolled[course.id] ? (
                    <>
                      ...
                      <button onClick={() => handleUnenroll(course.id)} className="bg-gray-200 text-purple-700 px-4 py-2 rounded mt-2 ml-2">Unenroll</button>
                    </>
                  ) : (
